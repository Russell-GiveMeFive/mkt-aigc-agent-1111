...
def inspect_image(role: str, raw_path: str) -> dict[str, Any]:
    ...
    with Image.open(path) as image:
        ...
        if has_alpha:
            ...
            nonzero_alpha_ratio = (
                (total_pixels - alpha_histogram[0]) / total_pixels if total_pixels else 0
            )
            partial_alpha_ratio = (
                sum(alpha_histogram[1:255]) / total_pixels if total_pixels else 0
            )
            opaque_alpha_ratio = alpha_histogram[255] / total_pixels if total_pixels else 0
        else:
            nonzero_alpha_ratio = 1.0
            partial_alpha_ratio = 0.0
            opaque_alpha_ratio = 1.0
        ...
        opaque_model_warning = role == "model" and not bool(
            has_alpha and alpha_min is not None and alpha_min < 255
        )
        ...
        return {
            ...
            "visible_aspect_ratio": (
                round(visible_width / visible_height, 8) if visible_height else None
            ),
            "nonzero_alpha_ratio": round(nonzero_alpha_ratio, 6),
            "partial_alpha_ratio": round(partial_alpha_ratio, 6),
            "opaque_alpha_ratio": round(opaque_alpha_ratio, 6),
            "extension_format_mismatch": path.suffix.lower().lstrip(".") not in {
                str(image.format).lower(),
                "jpg" if image.format == "JPEG" else str(image.format).lower(),
                "jpeg" if image.format == "JPEG" else str(image.format).lower(),
            },
            "opaque_model_warning": opaque_model_warning,
            "model_background_policy": (
                "rectangular background will remain; request transparent PNG or explicit background-removal permission"
                if opaque_model_warning
                else None
            ),
        }
