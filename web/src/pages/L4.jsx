  ...
  const generate = async () => {
    ...
    try {
      ...
      for (const id of sel) {
        ...
        const card = (assets.items || []).find((x) => x.fileId === id)
        jobsNew.push(await api.l4Generate({
          l3FileId: id,
          prompt: bound ? promptWithModel : promptNoModel,
          modelFileId: bound || undefined,
          productFileId: card?.productFileId || undefined, // L2 合成时记录的商品图 → H3 第 3 参考图（标签文字保真）
          videoOpts: vo,
        }))
      }
            ...
            {l3.filter((a) => a.usable !== false).map((a) => {
              ...
              return (
                <div key={a.fileId} className={`pickCard poster ${sel.includes(a.fileId) ? 'sel' : ''}`} style={{ cursor: 'pointer', position: 'relative' }} onClick={() => toggle(a.fileId)}>
                  ...
                  {a.productFileId && <span title={`商品图已关联（${a.productFileId}）· 生成视频时自动作为高清文字参考`} style={{ position: 'absolute', left: 6, bottom: 34, background: 'rgba(124,92,255,.9)', color: '#fff', borderRadius: 6, padding: '2px 7px', fontSize: 10.5, fontWeight: 600 }}>商品图✓</span>}
